...
- For send email, reply, reply all, forward, save draft, create file, append, update file, and Odoo post message: include min_words as an integer from 300 to 800. For FTPS upload, FTPS append, and FTPS update file: include min_words as an integer from 500 to 800. Do not write a long body or content; at most one short outline sentence. The soldier expands the prose. Do not put min_words on paths, recipients, subjects, or view-only actions.
- Exchange target: omit it to open the first email, or set target: 1, target: last, or a subject/sender substring. Never emit the string first email as a target value.
- Do not set Exchange recipient, cc, or bcc to the current role's short name or mailbox.
...
- Playwright tasks must be one line, use at least four numbered ops from the listed vocabulary (for example search, follow, scroll, extract), include Verify:, and end with Close the browser after verification. Do not emit REPLACE_ tokens or other placeholders. playwright-browser is public web only: do not use it for OWA or Odoo URLs, and do not name internal lab systems as browse targets. Do not mention playwright-browser inside an odoo-use or exchange-use task.
