...
- FTP server
  ...
  - Remote root: `/`. Each office role works in its own folder: `/hr`, `/accountancy`, `/manager`, `/programmer`.
...
4. `ftp-use`: Logs in to the company FTPS server and supports listing, uploading, downloading, appending, copying, moving, renaming, and deleting files, plus creating or deleting directories.
...
`<detail>` restates the op (`list a folder`, `upload a file`, `download a file`) and must not add extra semantics. Paths are the real remote POSIX paths `/<role>/...` (`/hr`, `/accountancy`, `/manager`, `/programmer`) and must stay under the role's FTP task path. Do not write `/ftp-root`. Prefer `upload`, `download`, `append`, and `copy` over `list`. For `upload` / `append` / `update file`, set `min_words` plus a short `topic`; the soldier expands the prose and transfers it.
