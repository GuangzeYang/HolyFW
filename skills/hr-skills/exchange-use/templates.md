...
Pass the quoted string to `opencode run`. Load this skill on the HR host. Values in `{...}` are JSON-like (no quotation marks around keys or values). Omit unused keys. If a prompt uses a short mailbox (`manager`), the agent must type `manager@ndrtest.local` into OWA — never the short name.
...
```text
opencode run "Use the exchange-use skill, open the Exchange mailbox, send email, {recipient: manager@ndrtest.local, cc: programmer, subject: Onboarding pack, body: Offer letter is attached., attachment: C:\Users\hr\Desktop\offer.pdf}"
```
...
```text
opencode run "Use the exchange-use skill, open the Exchange mailbox, reply all, {body: Thanks, I will follow up with HR-Private files.}"
```

```text
opencode run "Use the exchange-use skill, open the Exchange mailbox, forward, {target: first email, recipient: accountancy, body: FYI for payroll.}"
```
