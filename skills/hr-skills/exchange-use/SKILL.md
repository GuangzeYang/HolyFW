...
Left folder pane: search **Search Mail and People** (`Activate Search Textbox`), Favorites (**Inbox**, **Sent Items**, **Drafts**), then mailbox **hr** with **Inbox**, **Drafts**, **Sent Items**, **Deleted Items**, **Junk Email**, **Notes**.
...
1. Complete **view email**.
2. If toolbar **Flag for follow-up** is missing, the item cannot be flagged from this view (often the first list `option` is not a mail, or the pane is still compose). Stop without failing the whole mailbox session, or select a real message row first and retry once.
