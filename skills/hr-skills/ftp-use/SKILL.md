---
name: ftp-use
description: Use when logging in to the company FTPS server and listing, uploading, downloading, or deleting files, or creating or deleting directories. HR works under /ftp-root/hr. Use for explicit FTPS, FTP over TLS, or remote file transfer. Do not use for SMB shares or Exchange mail.
---
...
- Role home (default when the prompt does not name another folder): `/ftp-root/hr`
...
7. After login, `cwd` to `/ftp-root`, then to `/ftp-root/hr`. If `hr` is missing, create it, report that, and enter it.
...
- Default work stays under `/ftp-root/hr`. Create that folder if it is missing and report that.
...
Skip unused fields. Resolve `path` under `/ftp-root` first. If `path` is omitted, use `/ftp-root/hr`.
...
Task paths look like `/ftp-root/hr/notes.txt`. Use them as remote FTPS paths.

1. If the string already starts with `/ftp-root`, use it unchanged.
2. A bare file name is created or resolved under `/ftp-root/hr`.
3. Do **not** write under `/ftp-root/accountancy`, `/ftp-root/manager`, or `/ftp-root/programmer` unless the prompt names that exact path. Stick to `/ftp-root/hr`.
...
- Do not work outside `/ftp-root` or, by default, outside `/ftp-root/hr`.
