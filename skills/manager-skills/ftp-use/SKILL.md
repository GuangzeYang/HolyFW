---
name: ftp-use
description: Use when logging in to the company FTPS server and listing, uploading, downloading, or deleting files, or creating or deleting directories. Manager works under /ftp-root/manager. Use for explicit FTPS, FTP over TLS, or remote file transfer. Do not use for SMB shares or Exchange mail.
---
...
- Role home (default when the prompt does not name another folder): `/ftp-root/manager`
...
7. After login, `cwd` to `/ftp-root`, then to `/ftp-root/manager`. If `manager` is missing, create it, report that, and enter it.
...
- Default work stays under `/ftp-root/manager`. Create that folder if it is missing and report that.
...
Skip unused fields. Resolve `path` under `/ftp-root` first. If `path` is omitted, use `/ftp-root/manager`.
...
Task paths look like `/ftp-root/manager/spec-notes.txt`. Use them as remote FTPS paths.

1. If the string already starts with `/ftp-root`, use it unchanged.
2. A bare file name is created or resolved under `/ftp-root/manager`.
3. Do **not** write under `/ftp-root/hr`, `/ftp-root/accountancy`, or `/ftp-root/programmer` unless the prompt names that exact path. Stick to `/ftp-root/manager`.
...
- Do not work outside `/ftp-root` or, by default, outside `/ftp-root/manager`.
