---
name: ftp-use
description: Use when listing, uploading, downloading, appending, copying, moving, renaming, or deleting files and folders on the company FTPS server. Manager works under /manager. Use for explicit FTPS. Do not use for SMB shares or Exchange mail.
---
...
- Username: `ndrtest/manager` (forward slash). `manager` and `manager@ndrtest.local` also work. **`ndrtest\manager` returns 530.**
- Password: `Njupt@241`
...
Task paths are the **real remote paths**: `/manager/spec-notes.txt`, `/manager/reviews`. Do not write `/ftp-root/...`.
...
ftp.auth()
ftp.login('ndrtest/manager', '<password>')
ftp.prot_p()
ftp.set_pasv(True)
...
3. Order: `connect(..., timeout=30)` → `auth()` → `login('ndrtest/manager', password)` → `prot_p()` → `set_pasv(True)` → `OPTS UTF8 ON` → **`TYPE I`** → `cwd('/')`.
...
6. Then `cwd('/manager')`. If missing: `cwd('/')`, `mkd('manager')`, report that, `cwd('/manager')`. Do not create files at `/`.
...
10. On 530, stop. Do not retry `ndrtest\manager`.
...
- Default work stays under `/manager`. Do not write at `/` (root currently has `test.txt`; leave it).
...
Skip unused fields. If `path` is omitted, use `/manager`.
...
No server-side COPY. `RETR` to a local temp, then `stor_compat` to the destination (both under `/manager` unless the prompt named another allowed path).
...
Same-volume move works (`RNTO dest/renamed.txt`). Both sides stay under `/manager`. Success: destination exists, source is gone. Clash: `550 Cannot create a file when that file already exists` — stop.
...
|---|---|
| `/manager` or `/manager/` | `/manager` |
| `/manager/spec-notes.txt` | `/manager/spec-notes.txt` |
| `spec-notes.txt` | `/manager/spec-notes.txt` |
| `/` | `/` (list only; do not write) |

1. If the string starts with `/`, use it as the remote path.
2. A bare name is under `/manager`.
3. Stay under `/manager` unless the prompt already named another exact path.
4. `/ftp-root` is **not** on this server. If a leftover prompt still contains `/ftp-root/`, strip that prefix (`/ftp-root/manager/a.txt` → `/manager/a.txt`). Do not `cwd('/ftp-root')`.
5. `/hr`, `/accountancy`, `/programmer` are not present from a typical listing (`550`). Do not create them on a normal Manager task.
...
- Do not login as `ndrtest\manager`.
...
- Do not write at `/` or, by default, outside `/manager`.
