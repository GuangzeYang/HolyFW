...
Pass the quoted string to `opencode run`. Load this skill on the manager host. Values in `{...}` are JSON-like (no quotation marks around keys or values). Omit unused keys. If a prompt uses a short mailbox (`manager`), the agent must type `manager@ndrtest.local` into OWA — never the short name.
