# Accountancy opencode prompt templates

English only. Run on the Accountancy soldier host with the matching skill installed. Detailed copies live under `new_skill/accountancy-skills/`; `skills/accountancy-skills/` holds the deployed Accountancy skills.
...
Login initial directory is `/`. Task paths are real remote paths under `/accountancy`.
