---
name: smb-access
description: Use when creating, viewing, updating, copying, moving, renaming, or deleting files on the company SMB share through PowerShell. Accountancy may use accountancy, Public, and Exchange. Do not use a GUI explorer. Stop immediately on Access Denied.
---
...
Task paths look like `/Company_Data/accountancy/ledger.txt`. Convert to UNC:
...
- `/Company_Data/accountancy/a.txt` → `\\172.16.24.11\Company_Data\accountancy\a.txt`
...
# Allowed trees (Accountancy policy)

Write and delete only under:

- `\\172.16.24.11\Company_Data\accountancy`
- `\\172.16.24.11\Company_Data\Public`
- `\\172.16.24.11\Company_Data\Exchange`

Do **not** write, move-into, or delete under `HR-Private`, `IT-Dev`, or `Management` (Access Denied).

Other department folders may appear in a listing. **Do not use them.** Stick to accountancy / Public / Exchange.
...
3. Success: the listing includes `accountancy`, `Public`, and `Exchange`.
...
- Do not write to `HR-Private`, `IT-Dev`, or `Management` on a normal Accountancy task.
