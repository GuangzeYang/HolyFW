---
name: ftp-use
description: Use when logging in to the company FTPS server and listing, uploading, downloading, or deleting files, or creating or deleting directories. Accountancy works under /ftp-root/accountancy. Use for explicit FTPS, FTP over TLS, or remote file transfer. Do not use for SMB shares or Exchange mail.
---
...
- Role home (default when the prompt does not name another folder): `/ftp-root/accountancy`
...
7. After login, `cwd` to `/ftp-root`, then to `/ftp-root/accountancy`. If `accountancy` is missing, create it, report that, and enter it.
...
- Default work stays under `/ftp-root/accountancy`. Create that folder if it is missing and report that.
...
Skip unused fields. Resolve `path` under `/ftp-root` first. If `path` is omitted, use `/ftp-root/accountancy`.
...
Task paths look like `/ftp-root/accountancy/ledger.txt`. Use them as remote FTPS paths.

1. If the string already starts with `/ftp-root`, use it unchanged.
2. A bare file name is created or resolved under `/ftp-root/accountancy`.
3. Do **not** write under `/ftp-root/hr`, `/ftp-root/manager`, or `/ftp-root/programmer` unless the prompt names that exact path. Stick to `/ftp-root/accountancy`.
...
- Do not work outside `/ftp-root` or, by default, outside `/ftp-root/accountancy`.
