...
4. Username is usually pre-filled `ndrtest\accountancy`; if empty, fill `#username`. Do not use `#passwordText`.
5. Fill `#password` with `Njupt@241`.
6. Click `div.signinbutton` (visible text **sign in**). If missing, press Enter in `#password`.
7. Wait until the title contains `Mail - accountancy@ndrtest.local`. If the logon form is still shown, stop.

# Recipient rules (OWA people picker)

Fill every To / Cc / Bcc with the **full SMTP** address. `accountancy` means `accountancy@ndrtest.local`. Never a short alias.
