---
name: ftp-use
description: Use when listing, uploading, downloading, appending, copying, moving, renaming, or deleting files and folders on the company FTPS server. Accountancy works under /accountancy. Use for explicit FTPS. Do not use for SMB shares or Exchange mail.
---
...
- Username: `ndrtest/accountancy` (forward slash). `accountancy` and `accountancy@ndrtest.local` also work. **`ndrtest\accountancy` returns 530.**
- Password: `Njupt@241`
- Remote root / initial directory: `/`
- Role home: `/accountancy`
- SYST: `Windows_NT` (Microsoft FTP Service).
- No FQDN. Do not invent a host, port, or password.

Task paths are the **real remote paths**: `/accountancy/ledger.txt`, `/accountancy/invoices`. Do not write `/ftp-root/...`.
...
ftp.auth()
ftp.login('ndrtest/accountancy', '<password>')
ftp.prot_p()
ftp.set_pasv(True)
...
3. Order: `connect(..., timeout=30)` → `auth()` → `login('ndrtest/accountancy', password)` → `prot_p()` → `set_pasv(True)` → `OPTS UTF8 ON` → **`TYPE I`** → `cwd('/')`.
...
6. Then `cwd('/accountancy')`. If missing: `cwd('/')`, `mkd('accountancy')`, report that, `cwd('/accountancy')`. Do not create files at `/`.
...
10. On 530, stop. Do not retry `ndrtest\accountancy`.
...
- Default work stays under `/accountancy`. Do not write at `/` (root currently has `test.txt`; leave it).
...
Skip unused fields. If `path` is omitted, use `/accountancy`.
...
No server-side COPY. `RETR` to a local temp, then `stor_compat` to the destination (both under `/accountancy` unless the prompt named another allowed path).
...
Same-volume move works (`RNTO dest/renamed.txt`). Both sides stay under `/accountancy`. Success: destination exists, source is gone. Clash: `550 Cannot create a file when that file already exists` — stop.
...
|---|---|
| `/accountancy` or `/accountancy/` | `/accountancy` |
| `/accountancy/ledger.txt` | `/accountancy/ledger.txt` |
| `ledger.txt` | `/accountancy/ledger.txt` |
| `/` | `/` (list only; do not write) |

1. If the string starts with `/`, use it as the remote path.
2. A bare name is under `/accountancy`.
3. Stay under `/accountancy` unless the prompt already named another exact path.
4. `/ftp-root` is **not** on this server. If a leftover prompt still contains `/ftp-root/`, strip that prefix (`/ftp-root/accountancy/a.txt` → `/accountancy/a.txt`). Do not `cwd('/ftp-root')`.
5. `/hr`, `/manager`, `/programmer` are not present from a typical listing (`550`). Do not create them on a normal Accountancy task.
...
- Do not login as `ndrtest\accountancy`.
...
- Do not write at `/` or, by default, outside `/accountancy`.
