...
English only. Run on the Manager soldier host with the matching skill installed. Detailed copies live under `role_profiles/manager-skills/`; the deployed copy is `~/.config/opencode/skills/`.
...
Login initial directory is `/`. Task paths are real remote paths under `/manager`.
