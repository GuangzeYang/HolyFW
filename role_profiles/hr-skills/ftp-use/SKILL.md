---
name: ftp-use
description: Use when listing, uploading, downloading, appending, copying, moving, renaming, or deleting files and folders on the company FTPS server. HR works under /hr. Use for explicit FTPS. Do not use for SMB shares or Exchange mail.
---
...
- Username: `ndrtest/hr` (forward slash). `hr` and `hr@ndrtest.local` also work. **`ndrtest\hr` returns 530.**
- Password: `Njupt@241`
...
Task paths are the **real remote paths**: `/hr/notes.txt`, `/hr/onboarding`. Do not write `/ftp-root/...`.
...
3. Order: `connect(..., timeout=30)` → `auth()` → `login('ndrtest/hr', password)` → `prot_p()` → `set_pasv(True)` → `OPTS UTF8 ON` → **`TYPE I`** → `cwd('/')`.
...
6. Then `cwd('/hr')`. If missing: `cwd('/')`, `mkd('hr')`, report that, `cwd('/hr')`. Do not create files at `/`.
...
10. On 530, stop. Do not retry `ndrtest\hr`.
...
- Default work stays under `/hr`. Do not write at `/` (root currently has `test.txt`; leave it).
...
Skip unused fields. If `path` is omitted, use `/hr`.
...
No server-side COPY. `RETR` to a local temp, then `stor_compat` to the destination (both under `/hr` unless the prompt named another allowed path).
...
Same-volume move works (`RNTO dest/renamed.txt`). Both sides stay under `/hr`. Success: destination exists, source is gone. Clash: `550 Cannot create a file when that file already exists` — stop.
...
|---|---|
| `/hr` or `/hr/` | `/hr` |
| `/hr/notes.txt` | `/hr/notes.txt` |
| `notes.txt` | `/hr/notes.txt` |
| `/` | `/` (list only; do not write) |

1. If the string starts with `/`, use it as the remote path.
2. A bare name is under `/hr`.
3. Stay under `/hr` unless the prompt already named another exact path.
4. `/ftp-root` is **not** on this server. If a leftover prompt still contains `/ftp-root/`, strip that prefix (`/ftp-root/hr/a.txt` → `/hr/a.txt`). Do not `cwd('/ftp-root')`.
5. `/accountancy`, `/manager`, `/programmer` are not present (`550`). Do not create them on a normal HR task.
...
- Do not write at `/` or, by default, outside `/hr`.
