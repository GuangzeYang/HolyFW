---
name: smb-access
description: Use when creating, viewing, updating, copying, moving, renaming, or deleting files on the company SMB share through PowerShell. HR may use HR-Private, Public, and Exchange. Do not use a GUI explorer. Stop immediately on Access Denied.
---
...
Task paths look like `/Company_Data/HR-Private/staffing.txt`. Convert to UNC:
...
- `/Company_Data/HR-Private/a.txt` → `\\172.16.24.11\Company_Data\HR-Private\a.txt`
...
# Allowed trees (HR policy)

Write and delete only under:

- `\\172.16.24.11\Company_Data\HR-Private`
- `\\172.16.24.11\Company_Data\Public`
- `\\172.16.24.11\Company_Data\Exchange`

Do **not** write, move-into, or delete under `IT-Dev`, `Management`, or `Finance` (Access Denied).

`accountancy` may list for this account on the current ACL. **Do not use it** unless the prompt path is already inside an allowed tree. Stick to HR-Private / Public / Exchange.
...
3. Success: the listing includes `HR-Private`, `Public`, and `Exchange`.
...
- Do not write to `IT-Dev`, `Management`, `Finance`, or `accountancy` on a normal HR task.
