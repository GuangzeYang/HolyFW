...
COMMANDER_OPENCODE_JSON = Path(__file__).resolve().parent / "opencode.json"


def run_build(*, run_test: bool = False) -> int:
    code = install_commander_opencode(
        command_name="commander build",
        bundled_path=COMMANDER_OPENCODE_JSON,
    )
