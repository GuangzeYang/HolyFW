...
try:
    from common import strip_opencode_run_prefix, validate_task_id
...
def send_to_soldier(
    ...
    prompt = strip_opencode_run_prefix((task if task is not None else command) or "")
    payload = {
        "task_ref": task_ref,
        "task": prompt,
        "command": prompt,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--command",
        default="",
        help="deprecated alias of --task; prompt only, do not include opencode run",
    )
    parser.add_argument(
        "--task-id",
        default=None,
    ...
    parser.add_argument("--task", default="", help="OpenCode prompt for the soldier (no opencode run)")
    parser.add_argument(
        "--description",
        default=None,
...
def main() -> int:
    ...
    task_text = strip_opencode_run_prefix((args.task or "").strip())
    if not task_text and isinstance(args.description, str):
        task_text = strip_opencode_run_prefix(args.description.strip())
    if not task_text:
        task_text = strip_opencode_run_prefix((args.command or "").strip())
    if not task_text:
        logging.error("Provide --task with the OpenCode prompt (do not include opencode run)")
        return 1
