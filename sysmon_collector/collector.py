...
from sysmon_collector.elevate import is_elevated
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--skip-elevate",
        action="store_true",
        help="skip Administrator check (used by tests)",
    )
    ...
    if not args.skip_elevate and not is_elevated():
        print(
            "Sysmon collector requires an Administrator shell. "
            "Run sysmon-collect or python -m sysmon_collector manually as Administrator.",
            file=sys.stderr,
        )
        return 1
    try:
