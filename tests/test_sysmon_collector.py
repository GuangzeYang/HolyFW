...
class CollectorLoopTests(unittest.TestCase):
    ...
    def test_main_requires_administrator_when_not_elevated(self) -> None:
        with mock.patch("sysmon_collector.collector.os.name", "nt"):
            with mock.patch("sysmon_collector.collector.is_elevated", return_value=False):
                with mock.patch("sysmon_collector.collector.bootstrap_and_run") as boot:
                    self.assertEqual(collector.main([]), 1)
...
class SoldierSpawnTests(unittest.TestCase):
    ...
    def test_spawn_is_manual_only(self) -> None:
        import soldier.soldier as soldier

        with mock.patch("soldier.soldier.subprocess.Popen") as popen:
    ...
    def test_maybe_start_is_manual_only(self) -> None:
        import soldier.soldier as soldier
    ...
    def test_run_listen_does_not_spawn_sysmon(self) -> None:
        import soldier.soldier as soldier
...
class ElevateTests(unittest.TestCase):
    def test_non_windows_is_treated_as_elevated_for_tests(self) -> None:
        with mock.patch("sysmon_collector.elevate.os.name", "posix"):
            self.assertTrue(elevate.is_elevated())
